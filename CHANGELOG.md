...
- **Windows build:** Fix LNK2019 for `xbt_fseek64`: declare it with `extern "C"` in `platform.h` when included from C++, so the linker matches the C implementation in `platform_win.c`.
